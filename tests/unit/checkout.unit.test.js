  ...
  describe("Checkout", () => {
    ...
    it("should throw 404 error if cart is not present", async () => {
       ...
       */
        ...
        await expect(res).rejects.toThrowErrorMatchingSnapshot((err)=>{
    ...
    it("should update user balance and empty the cart on success", async () => {
      ...
      let initialwallet = userOneFinal.walletMoney;
      ...
      let finalwallet = userOneFinal.walletMoney;
      ...
      expect(initialwallet).toBeGreaterThan(finalwallet);
      //expect(true).toEqual(false);
    });
  });
});
